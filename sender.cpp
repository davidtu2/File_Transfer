...
unsigned long sendFile(const char* fileName){
	...
	while(!feof(fp)){
		...
		/* TODO: Send a message to the receiver telling him that the data is ready to be read (message of type SENDER_DATA_TYPE) */
		sndMsg.mtype = SENDER_DATA_TYPE;
		
		if(msgsnd(msqid, &sndMsg, sizeof(message) - sizeof(long), 0) < 0){
			perror("msgsnd in sendFile");
			exit(-1);
		}
		
		/* TODO: Wait until the receiver sends us a message of type RECV_DONE_TYPE telling us that he finished saving a chunk of memory */
...
void sendFileName(const char* fileName){
	...
	/* TODO: Create an instance of the struct representing the message containing the name of the file */
	fileNameMsg msg;
